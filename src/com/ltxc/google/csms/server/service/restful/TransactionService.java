...
public class TransactionService {
	...
	/**
	 * this one implement the pattern to avoid duplication transaction and AA
	 * service api load.
	 * 
	 ...
	 * @return
	 */
	public Response processTransaction(RestfulServiceBase restfulServiceBase,
			HttpServletRequest request, TransactionBase transaction,
			ITransactionService transactionService) {
		if (transaction == null) {
			String message = "Transaction Entity failed to be created from the http jason content.";
			logger.log(Level.SEVERE,
					"TransactionService:processTransaction - Error:" + message);
		...
		pr.setTransactionType(transaction.getTransactionType()
				.getTransactionTypeName());
		pr.setProcess_date(process_date);
		Status status = Response.Status.INTERNAL_SERVER_ERROR; // Internal
																// Server Error

		try {
			// double check whether device id is null in the deviceRegister,
			// otherwise, just set it to null
			User user = restfulServiceBase.getUser(request);
			logger.info("TransactionService:processTransaction -- submitted by user id:"
					+ user.getUserid()
					+ ". Transaction Type:"
			...
			if (transaction.getProcessActionCode() == 1) {
				...
				if(tbExist!=null)
				{
					...
					pr.setTransactionType(tbExist.getTransactionType().getTransactionTypeName());
					status = Response.Status.OK;
				}
				else
				{
					pr.setProcess_status(ProcessStatusEnum.NONE.getProcessStatusName());
					pr.setProcess_message(message);
					pr.setProcess_date(null);
				...
				long interval = 0;
				if (tbExist == null
						|| ((interval = this.getLapsedMinutes(tbExist
								.getProcessDate())) > this.resetInterval && !ProcessStatusEnum.COMPLETED
								.getProcessStatusName().equalsIgnoreCase(
										tbExist.getProcessStatus()))) {
					isLoad = true;
				...
				if (isLoad) {
					...
					if (loader != null) {
						...
						String message = "Failed to find the loader for shipping transaction: "
								+ transaction.toString();
						logger.log(Level.SEVERE,
								"TransactionService:processTransaction - Error:"
										+ message);
						restfulServiceBase.throwInternalErrorException(message);
					}
		...
		Response.ResponseBuilder builder = Response.ok(pr,
				MediaType.APPLICATION_JSON).status(status);
	...
	public long getLapsedMinutes(Date process_date) {
		...
		if (process_date != null) {
			...
			lapsedMinutes = (current.getTime() - process_date.getTime())
					/ SharedConstants.DATE_SECOND;
		}
